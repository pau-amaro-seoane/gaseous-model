      SUBROUTINE QUAD(ISUB,RX,QTIME,EKT)
*
*
...
*
*
*     PRINT*,' quad: cm7=',M(1),X(1),XD(1),P(1),Q(1),CM(7)
*     PRINT*,' quad: ',xr(1),w(1),r(1),ta(1),mij(1),name4(1)
...
*       Initialize diagnostic & COMMON variables.
   ...
   10 CONTINUE
      ...
      RATIO = 0.0
      SWITCH = .FALSE.
*       Define value to search minimum distance
      QRMIN = 1.D30
*
...
*
*     print*, ' quad: Energy,Rgrav,Rmax,tmax,Tstar,cm7=',energy,rgrav,
*    *       rmax,tmax,tstar,cm(7)
*     call flush(6)
*
...
*       Perform regularized integration until termination is reached.
   30 CONTINUE
*       We want to continue if going back here (IQ=0 added RSP)
      IQ = 0
      NSTEPO = NSTEP4
      CALL RCHAIN(IQ)
*
...
*
      DO 50 K = 1,3
          ...
          RDOT1 = RDOT1 + (X(J3) - X(J4))*(XD(J3) - XD(J4))
   50 CONTINUE
*
...
*
      ...
      SEMI1 = 1.0/SEMI1
      E2 = (1.0D0 - RB1/SEMI1)**2 + RDOT1**2/(SEMI1*MB1)
      IF(E2.GT.0.D0)THEN
      ECC1 = SQRT(E2)
      ELSE
      ECC1 = 1.D30
...
*       Determine nearest and most distant binary perturber.
      ...
      END IF
*
*       Determine orbital elements for c.m. with nearest binary perturber.(RSP)
      VRELCM = 0.D0
      RDOTCM = 0.D0
      RCM = 0.0D0
      RCM4 = 0.0D0
...
*
      DO 55 K = 1,3
          ...
          J3 = 3*(IM-1) + K
          J4 = 3*(IMAX-1) + K
          XCM = (M(I1)*X(J1)+M(I2)*X(J2))/MB
          XCMDOT = (M(I1)*XD(J1)+M(I2)*XD(J2))/MB
          RCM = RCM + (XCM - X(J3))**2
          VRELCM = VRELCM + (XCMDOT - XD(J3))**2
          RDOTCM = RDOTCM + (XCM - X(J3))*(XCMDOT - XD(J3))
*
          XCM2 = (M(IM)*X(J3)+M(IMAX)*X(J4))/MB1
          XCMDOT2 = (M(IM)*XD(J3)+M(IMAX)*XD(J4))/MB1
          RCM4 = RCM4 + (XCM - XCM2)**2
          VRELCM4 = VRELCM4 + (XCMDOT - XCMDOT2)**2
          RDOTCM4 = RDOTCM4 + (XCM - XCM2)*(XCMDOT - XCMDOT2)
   55 CONTINUE      
      RCM = DSQRT(RCM)
      RCM4 = DSQRT(RCM4)
*
      SEMICM = 2.0D0/RCM - VRELCM/(MB+M(IM))
      SEMICM = 1.0/SEMICM
      EBCM = -0.5D0*MB*M(IM)/SEMICM
      E2 = (1.0D0-RCM/SEMICM)**2+RDOTCM**2/(SEMICM*(MB+M(IM)))
      IF(E2.GT.0.D0)THEN
      ECCCM = SQRT(E2)
      ELSE
      ECCCM = 1.D30
      END IF
*       Determine orbital elements between c.m.s of two binaries
      SEMICM4 = 2.0D0/RCM4 - VRELCM4/(MB+MB1)
      SEMICM4 = 1.0/SEMICM4
      EBCM4 = -0.5D0*MB*MB1/SEMICM4
...
*
   60 CONTINUE
*
      IF(MOD(ITRY,100).EQ.0)THEN
      PRINT*,' ==========================================='
      PRINT*,' R1-6=',RB,R13,SQRT(R2(I1,I4)),SQRT(R2(I2,I3)),
     *    R24,RB1
      PRINT*,' quad: ITRY,NSTEP4,RMAX4,RM,RCM,TIME4=',
     * ITRY,NSTEP4,RMAX4,RM,RCM,TIME4
      PRINT*,' quad: SEMI,SEMI1,SEMICM=',SEMI,SEMI1,SEMICM
      PRINT*,' quad: ECC,ECC1,ECCCM=',ECC,ECC1,ECCCM
      CALL FLUSH(6)
      END IF
*
...
*         GO TO 30
*     END IF
*       Check for two binaries or all four bodies 
*        which are marginally bound to each other
*       Terminate such configuration if minimum number of steps done 1000*ITRY
*       even if separation still not gt RMAX4
*
      IF(ITRY.GT.500.AND.EBCM4.GT.-0.01*EKT)THEN
      IF(MOD(ITRY,100).EQ.0)THEN
      PRINT*,' quad: Info EBCM4 > -0.01 EKT'
      PRINT*,' MB,MB1,RB,RB1,EBCM4,SEMICM4=',MB,MB1,RB,RB1,EBCM4,SEMICM4
      PRINT*,' I1-4,M(I1-4)=',I1,I2,I3,I4,M(I1),M(I2),M(I3),M(I4)
      PRINT*,' IM,IMAX,RM,RMAX=',IM,IMAX,RM,RMAX
      END IF
      RDD = MIN(RM,RMAX)
      IF(SEMI/RDD.LT.1.D-3.AND.SEMI1/RDD.LT.1.D-3)THEN
      PRINT*,' quad: Term due to EBCM4 > -0.01 EKT,SEMI,SEMI1,RDD=',
     *  SEMI,SEMI1,RDD,RM,RMAX
      GO TO 70
      END IF
      END IF
*
*       Experimental keep going if SEMI<0 (R.Sp.Oct.2000)
      IF (SEMI.LT.0.D0)GO TO 30   
*
      IF(SEMI.GT.SEMI1)THEN
      XMUE = 8.D0*MB1/MB
      RMAX4 = 3.6D0*SEMI*(1.D0+ECC)*(1.D0+1.D1*(XMUE)**C13)
      ELSE
      XMUE1 = 8.D0*MB/MB1
      ECC1 = SQRT((1.0D0 - RB1/SEMI1)**2 + RDOT1**2/(SEMI1*MB1)) 
      RMAX4 = 3.6D0*SEMI1*(1.D0+ECC1)*(1.D0+1.D1*(XMUE1)**C13) 
      END IF
*
      IF(SEMI1.LT.0.D0.AND.SEMICM.GT.SEMI)THEN
      XMUE1 = 8.0D0*M(IMAX)/(MB + M(IM))
      RMAX4 = 3.6D0*SEMICM*(1.D0+ECCCM)*(1.D0+1.D1*(XMUE1)**C13) 
*       Check whether triple is in rare state of near parabolic expansion
         IF(SEMICM.GT.1.D3*SEMI.AND.ECCCM.GT.0.999D0)THEN
      XMUE = 8.D0*M(IM)/MB
      RMAXSA = RMAX4
      RMAX4 = 3.6D0*SEMI*(1.D0+ECC)*(1.D0+1.D1*(XMUE)**C13)
         PRINT*,' quad: rare case of near parabolic expansion ',
     *   ' RMAX4 reduced from ',RMAXSA,' to ',RMAX4
         END IF
*
      END IF
*              
*     RATIO = RM/SEMI/(1.D0+ECC)
*     IF(RATIO.LT.5.0.AND.ITRY.LT.100)GO TO 30
*       Check for two binaries which are marginally bound to each other
*       Terminate such configuration if minimum number of steps done 1000*ITRY
*       even if separation still not gt RMAX4
*
*       Check whether one star is far out.
      IF(RMAX.LT.RMAX4)GO TO 30
*
*       Check triple left over
      IF(ECCCM.LT.1.D0)THEN
      XCRIT = 2.8D0*SEMI*((1.D0+M(IM)/MB)*(1.D0+ECCCM)/
     *    DSQRT(1.D0-ECCCM))**4.D-1
*       Pericentre Distance of outer binary
      YCRIT = SEMICM*(1.D0-ECCCM)
*       Terminate if stable triple acc. to Mardling / Aarseth is left over
      IF(YCRIT.GT.XCRIT)THEN
      PRINT*,' quad: Term MA-crit Rpcrit,Xcrit=',YCRIT,XCRIT,ITRY
      GO TO 70
      END IF
*       If triple is unstable check how long lived it was
      PRINT*,' quad: Cont MA-crit Rpcrit,Xcrit=',YCRIT,XCRIT,ITRY
*      Terminate triple if too long-lived
      IF(RMAX.GT.5.D0*RMAX4)THEN
      PRINT*,' quad: Term due to RMAX=',RMAX
      GO TO 70
      END IF
*
      END IF                    
...
*       Worst case termination
      IF(ITRY.GT.500)THEN
      PRINT*,' quad: Term due to worst case ITRY=',ITRY,
     *  ' marginally stable triple? '
      GO TO 70           
      END IF
*
*       Check second largest distance agains RMAX4
      IF(RM.LT.RMAX4)THEN
      PRINT*,' quad: Cont due to RM,RMAX4=',RM,RMAX4,ITRY
      GO TO 30
      ELSE
      PRINT*,' quad: Term due to RM,RMAX4=',RM,RMAX4,ITRY
      GO TO 70
      END IF     
*
...
*       Check optional print diagnostics of four-body regularization.
   ...
   90     FORMAT (' END QUAD   ',4I3,'  RB =',1PE8.1,'  R13 =',E8.1,
     &              '  R24 =',E8.1,'  RG =',E8.1,'  TC =',D12.5,'  #',
     &                 I8,I4,'  DB =',1P,E13.5,'  EC =',F6.3)
          WRITE (6,91) QTIME,QRMIN,DSQRT(R2(I1,I2)),DSQRT(R2(I1,I3)),
     &      DSQRT(R2(I1,I4)),DSQRT(R2(I2,I3)),DSQRT(R2(I2,I4)),
     &      DSQRT(R2(I3,I4))
   91     FORMAT (' END QUAD T,Min. Dist. ',1P,2D13.5,' R1-6=',6D13.5)
      END IF
*
...
*       Update current time unless termination and set subsystem index.
  100 CONTINUE
      ISUB = ITERM
*       Save RGRAV for diagnostics in qumain
      RX = RGRAV
      RETURN
*
      END
